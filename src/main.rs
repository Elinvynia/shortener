...
async fn main() {
    ...
    app.at("/")
        .get(|req: Request<State>| async move { Ok(req.state().render("index.tera")) });
...
impl State {
    ...
    fn render(&self, name: &str) -> Response {
        ...
        Response::builder(200).content_type(mime::HTML).body(render).build()
    }

    fn render_with_context(&self, name: &str, context: &Context) -> Response {
        let render = self.tera().render(name, context).unwrap_or_else(|err| err.to_string());
